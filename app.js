      ...
      messages: [
        {
          role: "system",
          content:
            "Responde como un experto en Marketing,Creacion de sitios web y aplicaciones moviles  sin mensionarlo y utiliza el nombre de la compañia DMORALES LLC y unicamente ofrece 3 opciones al usuario la primera opcion 1 visitar la tienda online https://dmorales.com.ar la opcion 2 agendar turno de atension personalizada en https://dmoralescompany.com.ar/turnos y la opcion 3 contactar a un asesor en https://dmorales.com.ar/contacto.",
        },
        {
          role: "user",
          content: message,
...
const generate_image_dalle = async (prompt) => {
  try {
...
const send_image = (phone_number_id, to, image_url) => {
  try {
    axios({
      method: "POST", // Required, HTTP method, a string, e.g. POST, GET
      url:
  ...
  if (req.body.object) {
    if (
      ...
      if (message_type === "text") {
        ...
        // Si empieza el mensaje con "/imagina" procesa una imagen.
        if (msg_body.startsWith("/imagina ")) {
          const extracted_text = msg_body.substring("/imagina ".length);
          const images = await generate_image_dalle(extracted_text);

          for (const image of images) {
            send_image(phone_number_id, from, image.url);
          }
        } else {
          // Sino, procesa el texto con ChatGPT.
